...
export default class Collapsible extends Component {
        ...
        if (typeof easing === 'string') {
            ...
            for (let i = 0; i < ANIMATED_EASING_PREFIXES.length; i++) {
                ...
                if (easing.substr(0, prefix.length) === prefix) {
                    ...
                    prefix =
                        prefix.substr(4, 1).toLowerCase() + prefix.substr(5);
            ...
            if (!easing) {
                throw new Error(
                    'Invalid easing type "' + this.props.easing + '"'
                );
            }
        }
    ...
    render() {
        ...
        // 1. Always apply the style to the parent Animated.View for that Removed the condition to apply style to parent.
        const style = {
            overflow: 'hidden',
            height: height,
        };
        ...
        return (
            <Animated.View
                style={style}
                pointerEvents={
                    !enablePointerEvents && collapsed ? 'none' : 'auto'
                }
            >
                <Animated.View
                    ref={this._handleRef}
